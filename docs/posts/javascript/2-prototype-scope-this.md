...
__prototype__:每个函数都具有的属性，且只有函数才有的属性。函数的prototype所指向的对象，就是该函数作为构造函数创建的*实例的原型对象*。

__proto__:每一个JavaScript对象（除nul）都具有的属性，这个属性会指向该*对象的原型*。

__constructor__:每一个原型都具有的属性，该属性指向关联这个*原型的构造函数*。
...
### 闭包的应用
数据封装: 内部保存数据,抛出get set

防抖节流 柯里化 异步请求的回调 模块化方案
